                ...
                if not clustered[x][y]:
                    ...
                    if newclust != []:
                        ...
                        if (len(clusters) % 1000 == 0):
                            print(
                                f'\tat {round(100*(x*ywidth + y) / (xwidth * ywidth))}% found'
...
def loadPatterns(patternfilelist):
    ...
    for file in patternfilelist:
        try:
            ...
            h /= count * 255
            chosen_count[file] = 0
            patterns.append({"brightness": h, "pixels": paxels, "type": re.findall(
...
def getMatchingPattern(typeList, bright):
    ...
    minpattern = sorted(filter(lambda d: d["type"] in typeList, patterns),
                        key=lambda item: item["brightness"], reverse=True)[0]
...
# he following 6 defs b1v1 , blendvalue, getPatternPixel, Pixelshiftbrightness,
# pfilter and greenTransparent are unknown in terms of functionality
